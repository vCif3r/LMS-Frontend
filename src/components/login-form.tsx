...
export function LoginForm({
  ...
  const loginSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:3000/auth/login", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // Corregido el tipo de contenido
        },
        body: JSON.stringify({ email, password }), // Pasar los valores del formulario
      })
  ...
  return (
    <form className={cn("flex flex-col gap-6", className)} onSubmit={loginSubmit} {...props}>
